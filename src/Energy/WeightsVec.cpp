...
WeightsVec& WeightsVec::operator+=(WeightsVec const& other)
{
    ...
    m_unaryWeight += other.m_unaryWeight;
...
Weight WeightsVec::sqNorm() const
{
    ...
    sqNorm += m_unaryWeight * m_unaryWeight;
    for (size_t i = 0; i < m_pairwiseWeights.size(); ++i)
...
std::ostream& operator<<(std::ostream& stream, WeightsVec const& weights)
{
    ...
    stream << std::setw(10) << weights.m_unaryWeight;
    ...
    stream << std::setw(10) << weights.m_featureWeight;
    stream << std::endl << std::endl;
...
bool WeightsVec::write(std::string const& filename) const
{
    ...
    if(out.is_open())
    {
        ...
        out.write(reinterpret_cast<const char*>(&m_unaryWeight), sizeof(m_unaryWeight) * noUnaries);
...
bool WeightsVec::read(std::string const& filename)
{
    ...
    if(in.is_open())
    {
        ...
        assert(noUnaries == 1);
        in.read(reinterpret_cast<char*>(&m_unaryWeight), sizeof(m_unaryWeight) * noUnaries);
        size_t noPairwise;
